...
├── .thumb-cache/      ← generato automaticamente, miniature ridotte delle immagini (richiede "sharp", vedi punto 9)
└── public/
...
Non è obbligatoria: se non è installata (o l'installazione fallisce), il server continua a funzionare normalmente e mostra semplicemente il file originale al posto della miniatura ridotta — nessun errore, nessuna funzione rotta, solo l'ottimizzazione in meno. Su Raspberry Pi OS a 64 bit (`uname -m` → `aarch64`) con Node.js recente scarica un binario già pronto in pochi secondi, senza compilare nulla; su un OS a 32 bit potrebbe richiedere qualche minuto in più.

Dopo l'installazione riavvia il server perché il cambiamento abbia effetto: `pm2 restart homelab` (o di nuovo `node server.js` se non hai ancora configurato PM2 al punto successivo).
...
### Le miniature delle foto restano a piena risoluzione (o non compaiono)
- Verifica che `sharp` sia installato: `cd ~/homelab && npm list sharp`

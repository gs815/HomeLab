...
├── .thumb-cache/      ← generado automáticamente, miniaturas reducidas de las imágenes (requiere "sharp", ver punto 9)
└── public/
...
#### Miniaturas reducidas para las fotos (opcional, recomendado)

En la sección Cloud, la lista de archivos muestra una miniatura real de las imágenes en lugar de un icono genérico. Para que sea una **miniatura realmente reducida** (unos pocos KB, en lugar de descargar la foto entera solo para verla en pequeño) hace falta la librería `sharp`:

```bash
cd ~/homelab && npm install sharp
```

No es obligatoria: si no está instalada (o la instalación falla), el servidor sigue funcionando con normalidad y simplemente muestra el archivo original en lugar de la miniatura reducida — ningún error, ninguna función rota, solo falta la optimización. En Raspberry Pi OS de 64 bits (`uname -m` → `aarch64`) con Node.js reciente descarga un binario ya compilado en pocos segundos, sin compilar nada; en un SO de 32 bits puede tardar algo más.

Después de instalarla, reinicia el servidor para que el cambio surta efecto: `pm2 restart homelab` (o de nuevo `node server.js` si aún no has configurado PM2, ver el siguiente paso).
...
### Las miniaturas de las fotos se quedan a resolución completa (o no aparecen)
- Comprueba que `sharp` esté instalado: `cd ~/homelab && npm list sharp`
- Si falta: `npm install sharp`, luego `pm2 restart homelab`
- Revisa los logs para ver el motivo exacto: `pm2 logs homelab`
- No es un problema bloqueante: sin `sharp` el sistema muestra igualmente el archivo original, solo que más pesado de cargar
